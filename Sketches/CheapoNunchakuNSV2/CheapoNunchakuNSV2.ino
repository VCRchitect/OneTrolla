...
void updateButtonStatus(int sensorValueY, int sensorValueX, int clickStateC, int clickStateZ) {
  ...
  } else if (state == DPAD_MODE && clickStateC == 0 && clickStateZ == 0) {
    ReportData.RX = sensorValueX;
    ReportData.RY = 255 - sensorValueY;  // Invert the Y-axis value
  ...
  } else if (clickStateC == 1 && clickStateZ == 1) {
    ReportData.LX = 128;
    ReportData.LY = 128;
    ReportData.RX = 128;
    ReportData.RY = 128;
...
void handleAccel(uint16_t aY, uint16_t aX, uint16_t clickStateC, uint16_t clickStateZ) {
  if (clickStateC == 0 || clickStateZ == 0) {
    ...
    if (state == DPAD_MODE && aY <= accel3Threshold) {
      buttonStatus[6] = HIGH;  // BUTTONUP
    } else {
      buttonStatus[6] = LOW;
    }
    if (state == DPAD_MODE && aY >= accel4Threshold) {
      buttonStatus[7] = HIGH;  // BUTTONUP
    } else {
      buttonStatus[7] = LOW;
    }
    if (state == DPAD_MODE && aX <= accel3Threshold) {
      buttonStatus[8] = HIGH;  // BUTTONUP
    } else {
      buttonStatus[8] = LOW;  // BUTTONUP
    }

    if (state == DPAD_MODE && aX >= accel4Threshold) {
      buttonStatus[9] = HIGH;  // BUTTONUP
    } else {
      buttonStatus[9] = LOW;  // BUTTONUP
    }
  }
}
